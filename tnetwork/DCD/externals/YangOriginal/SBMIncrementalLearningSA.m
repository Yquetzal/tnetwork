function net=SBMIncrementalLearingSA(graph,K,net)
% SBM Incremental Learning using Simulated Annealing 
% usage: Z=HMMStaticPartitionSA(graph,K,net)
...
%  net.Z: the cluster lable at previous time and the initial label at the  current time
...
if isempty(net.Z)
    %net.Z is empty we thought we did the clustering without previous time
    net=SBMStaticPartitionSA(graph,K,net);
    return;
elseif size(net.Z,2)==1
    Z(:,1)=net.Z;
    Z(:,2)=net.Z;
else
    disp('you have given the initial community assignments at the current time');
end

if issymmetric(W)
    W=formatizeW(W);%  see definition of formatize W at the bottom
end
...
if isempty(net.wthreshold)&& strcmp(net.type,'simi')
     disp('w threshold are set to default which could lead to bad result');
    wthreshold=[0.3 0.7];
...
if isempty(net.paraP)
    disp('hyperparameters for P are set to default which could lead to bad result');
    paraP=[1 4 2];
    net.paraP=[1 4 2];
else
    paraP=net.paraP;
...
if isempty(net.paraA)
     disp('hyperparameters for A are set to default which could lead to bad result');
    paraA=[1 2];
    net.paraA=[1 2];
else
    paraA=net.paraA;
...
if isempty(net.Temp) || isempty(net.N)
     disp('parameters for annealing are set to default which could lead to bad result');
    Temp=[0];
     N=[100];
     net.Temp=Temp;
     net.N=N;
...
ga=ones(1,K);%% the hyperparameter for pi
A=paraP(1)*ones(K,K)+paraP(2)*eye(K);% the hyperparameter for P matrix, the matrix of \alpha
B=paraP(3)*ones(K,K);% the hyperpameter for P matrix, the matrix of \beta
M=paraA(1).*ones(K,K)+paraA(2).*eye(K);% the hyperparameter for A matrix, the matrix of \mu
MS=sum(M,2);
