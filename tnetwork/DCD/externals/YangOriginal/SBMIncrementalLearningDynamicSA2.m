function net=SBMIncrementalLearingDynamicSA2(graph,K,net)
...
%   this version is different from SBMIncrementalLearingDynamicSA in that
%   we add the term StateNumOfNewNodes i.e., i\in U_t stated in the paper
if nargin<3
    help SBMIncrementalLearingDynamicSA2
end
...
for i=1:n
  ...
  if isempty(lip)==0
      ...
      s2=Z{2}(i,2);
      StateNumOfNewNodes(s2)=StateNumOfNewNodes(s2)+1;
  end
end
    ...
    for it=1:N(Tk)
        change=n;        
                ...
                for j=1:n
                   ...
                   NewStateNumOfNewNodes=zeros(K,K);
                   m=-inf;
                   shifK=[];
                   for k=1:K
                        CondPro=zeros(1,K);
                         newk=k;                   
                        [NewStateNumAtCurrentTime(k,:) NewPWStateState(:,:,k)  NewNWStateState(:,:,k)  NewMWStateState(:,:,k)  NewStateState(:,:,k)  NewSelfStateState(:,:,k)   NewStateToState(:,:,k)  NewStateToAll(:,k) NewStateNumOfNewNodes(k,:)]=update(StateNumAtCurrentTime, PWStateState, NWStateState,MWStateState, StateState,SelfStateState,StateToState,StateToAll,StateNumOfNewNodes, i,newk,Z,PW,NW,MW,n,K,type);
                        ...
                        o=sum(gammaln(NewStateNumOfNewNodes(k,:)+ga))+sum(sum(gammaln(NewStateToState(:,:,k)+M)))-sum(gammaln(NewStateToAll(:,k)+MS))+(sum(sum(betaln(PWS+MWS+A, NWS+MWS+B)))+sum(betaln(PWSD+MWSD+diag(A), NWSD+MWSD+diag(B))))/2;%gammaln(NewWStateState+A)+gammaln(NewNWStateState+B)-gammaln(NewWStateState+NewNWStateState+A+B)));
                        if o==inf
                            disp('error');
                        end
                        if o>m
        ...
        %% Gibbs Sampling
                     if Tep~=0
                            ...
                            if newk==ok
                                ...
                                StateNumOfNewNodes=NewStateNumOfNewNodes(newk,:);
                             end
        %% ICM  
                     else
...
function [NewStateNumAtCurrentTime NewPWStateState NewNWStateState NewMWStateState NewStateState NewSelfStateState NewStateToState NewStateToAll NewStateNumOfNewNodes]=update(StateNumAtCurrentTime, PWStateState, NWStateState,MWStateState,StateState,SelfStateState,StateToState, StateToAll, StateNumOfNewNodes, i, newk, Z,PW,NW,MW, n,K,type)
...
if newk==ok
   ...
   NewStateNumOfNewNodes=StateNumOfNewNodes;
else
   NewStateNumAtCurrentTime=StateNumAtCurrentTime;
   ...
   elseif strcmp(type, 'simi')
    ...
    gi=Z{t}(i,1);% global index at current time of node i 
    lpi=find(Z{t-1}(:,1)==gi);%% location at previous time of node i
    if isempty(lpi)==0
       pki=Z{t-1}(lpi,2);
       ...
       NewStateNumOfNewNodes=StateNumOfNewNodes;
        ...
        NewStateNumOfNewNodes=StateNumOfNewNodes;
        NewStateNumOfNewNodes(newk)=StateNumOfNewNodes(newk)+1;
        NewStateNumOfNewNodes(ok)=StateNumOfNewNodes(ok)-1;
    end
end
...
elseif length(find(NewStateNumOfNewNodes<0))>0
    disp('error at line 436');
end
