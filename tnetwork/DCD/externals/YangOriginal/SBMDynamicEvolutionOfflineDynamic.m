function net=SBMDynamicEvolutionOfflineDynamic(SocNet,K,net,Indexx)
        ...
        for t=1:T
                ...
                for j=1:n
                   ...
                   for k=1:K
                        CondPro=zeros(1,K);
                         newk=k;                   
                        [NewStateNumAtCurrentTime(k,:) NewPWStateState(:,:,k)  NewNWStateState(:,:,k)  NewMWStateState(:,:,k)  NewStateState(:,:,k)  NewSelfStateState(:,:,k)  NewAllStateState(:,:,k)  NewStateToState(:,:,k)  NewStateToAll(:,k)]=update(StateNumAtTime, PWStateState, NWStateState,MWStateState, StateState,SelfStateState,AllStateState,StateToState,StateToAll, i,t,newk,Z,PW,NW,MW,n,K,T,type,Index);
                        ...
                        o=sum(gammaln(NewStateNumAtOne+ga))+sum(sum(gammaln(NewStateToState(:,:,k)+M)))-sum(gammaln(NewStateToAll(:,k)+MS))+(sum(sum(betaln(PWS+MWS+A, NWS+MWS+B)))+sum(betaln(PWSD+MWSD+diag(A), NWSD+MWSD+diag(B))))/2;%gammaln(NewWStateState+A)+gammaln(NewNWStateState+B)-gammaln(NewWStateState+NewNWStateState+A+B)));
                        if o==inf
                            disp('error');
                        end
                        if o>m
...
function [NewStateNumAtCurrentTime NewPWStateState NewNWStateState NewMWStateState NewStateState NewSelfStateState NewAllStateState NewStateToState NewStateToAll]=update(StateNumAtTime, PWStateState, NWStateState,MWStateState,StateState,SelfStateState,AllStateState,StateToState, StateToAll, i, t, newk, Z,PW,NW,MW, n,K,T,type,Index)
