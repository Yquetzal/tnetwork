...
%   until no such moves improve quality; the snapshot_affiliations found to that
...
while (isa(M,'function_handle')) %loop around each "pass" (in language of Blondel et al) with B function handle  
    clocktime=clock;
    mydisp(['Merging ',num2str(length(y)),' snapshot_affiliations  ',datestr(clocktime)]);
    Sb=S;
    yb=[];
    while ~isequal(yb,y)
...
while ~isequal(Sb,S2) %loop around each "pass" (in language of Blondel et al) with B matrix    
    clocktime=clock;
    mydisp(['Merging ',num2str(max(y)),' snapshot_affiliations  ',datestr(clocktime)]);
...
%Computes new aggregated network (snapshot_affiliations --> nodes)
