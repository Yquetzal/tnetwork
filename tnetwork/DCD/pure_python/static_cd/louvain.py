...
def partition_at_level(dendrogram, level) :
    ...
    See Also
    --------
    runLouvain which directly combines partition_at_level and generate_dendrogram to obtain the partition of highest QualityFunction
...
def modularity(partition, graph) :
    """Compute the QualityFunction of a partition of a graph

    Parameters
    ----------
    partition : dict
       the partition of the nodes, i.e a dictionary where keys are their nodes and values the snapshot_affiliations
    graph : networkx.Graph
       the networkx graph which is decomposed

    Returns
    -------
    QualityFunction : float
       The QualityFunction
    ...
    if links == 0 :
        raise ValueError("A graph without link has an undefined QualityFunction")
...
def best_partition(graph, partition = None) :
    """Compute the partition of the graph nodes which maximises the QualityFunction
    (or try..) using the Louvain heuristices

    This is the partition of highest QualityFunction, i.e. the highest partition of the dendrogram
    ...
    partition : dict, optionnal
       the algorithm will start using this partition of the nodes. It's a dictionary where keys are their nodes and values the snapshot_affiliations

    Returns
    -------
    partition : dictionnary
       The partition, with snapshot_affiliations numbered from 0 to number of snapshot_affiliations
    ...
    References
    ----------
    .. 1. Blondel, V.D. et al. Fast unfolding of snapshot_affiliations in large networks. J. Stat. Mech 10008, 1-12(2008).
    ...
    >>> partition = community.runLouvain(G)
    >>>  #drawing
...
def generate_dendrogram(graph, part_init = None) :
    ...
    #print("first:"+str(new_mod))

    while True :
        __one_level(current_graph, status)
        new_mod = __modularity(status)
        #print("next:"+str(new_mod))
...
def __modularity(status) :
    """
    Compute the QualityFunction of the partition of the graph faslty using status precomputed
    """
...
def main() :
    ...
    try :
        ...
        print("find the snapshot_affiliations in graph filename and display the dendrogram")
        print("Parameters:")
